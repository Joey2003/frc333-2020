...
public class BTMacroRecord {
	...
	public void record(RobotChassis m_chassis, RobotShooter m_shooter, RobotTurret m_turret, RobotHood m_hood) throws IOException
	{
		if(writer != null)
		{
		...
		//drive motors
		writer.append("," + m_chassis.getLeft());
		writer.append("," + m_chassis.getRight());
		
		//shooter motor
		writer.append("," + m_shooter.getShooter());
		writer.append("," + m_shooter.getThroat());
		writer.append("," + m_shooter.getConveyer());
		writer.append("," + m_shooter.getCycler());
		writer.append("," + m_chassis.rollers().get());

		//turret motor
		writer.append("," + m_turret.get());

		//hood motor
		writer.append("," + m_hood.get());
		/*
		...
		writer.append("," + m_chassis.getSolenoids() + "\n");
